...
/**
 *  下载管理类 封装NSURLSessionDLTask
 */
@interface A_TYDLSessionManager : NSObject<NSURLSessionDownloadDelegate>
...
@property (nonatomic, copy) void (^C_backgroundSessionCompletionHandler)();

// 后台下载完成后调用 返回文件保存路径filePath
@property (nonatomic, copy) NSString *(^C_backgroundSessionDLCompleteBlock)(NSString *C_DLURL);
...
// 是否已经下载
- (BOOL)B_isDLCompletedWithDLModel:(A_TYDLModel *)dC_ownloadModel;
